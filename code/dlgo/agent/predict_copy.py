...
# tag::dl_agent_serialize[]
    def serialize(self, kerasfile):
        #h5file.create_group('encoder')
        #h5file['encoder'].attrs['name'] = self.encoder.name()
        #h5file['encoder'].attrs['board_width'] = self.encoder.board_width
        #h5file['encoder'].attrs['board_height'] = self.encoder.board_height
        #h5file.create_group('model')
        #不知道怎么保存，搞不定，就不搞了，好不好？！
        # encoder = {}
        # encoder["name"] = self.encoder.name()
        # encoder["board_width"] = self.encoder.board_width
        # encoder["board_height"] = self.encoder.board_height
        #print(self.model.summary())
        
        self.model.save(kerasfile)
...
def load_prediction_agent(kerasfile):
    #model = kerasutil.load_model_from_hdf5_group(h5file['model'])
    model = keras.saving.serialize_keras_object(kerasfile)
