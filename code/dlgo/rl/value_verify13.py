...
class ValueAgent(Agent):
    ...
    def select_move(self, game_state):
        ...
        # num_moves = len(moves)
        board_tensors_pre = np.array(board_tensors)

        print("cccaaaaaaaaaaaaaaaaaddd")
        print(board_tensors_pre.shape)
        #print(input_tensor[0].shape)
        board_tensors = tf.transpose(board_tensors_pre, perm=[0, 2, 3, 1])  # 注意 perm 参数的顺序
