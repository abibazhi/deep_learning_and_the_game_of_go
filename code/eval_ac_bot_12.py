...
def main():
    ...
    parser.add_argument('--num-games', '-n', type=int, default=500)
    ...
    color1 = Player.black
    print(f"请注意本互搏程序，需要手工指定board-size，否则缺省值是9")
    print(f"就在eval_ac_bot_12.py这个评估文件中，大写BOARD_SIZE")
    print(f"注意两个agent互搏时，board-size必须要一样的。")
    for i in range(args.num_games):
        ...
        if game_record.winner == color1:
            wins += 1
            print(f"agent1 wins total:{wins}")
        else:
            losses += 1
            print(f"agent2 wins total:{losses}")
        color1 = color1.other
