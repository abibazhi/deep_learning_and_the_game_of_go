...
def predict():
    ...
    #data_generator = DataGenerator(data_directory, files)
    #data_generator = processor.load_go_data('train', num_games, use_generator=True)
    print("000000000000000")
    
    # sampler = Sampler(data_dir='../data')
    # data = sampler.draw_data(data_type="train", num_samples=1000)
    # data_generator3 = DataGenerator3('../data', data)
    # dataset = data_generator3.generate(batch_size, num_classes)

    #print(type(dataset))

    features_path = "./code/data/KGS-2007-19-11644-train_features_100.npy"
    labels_path = "./code/data/KGS-2007-19-11644-train_labels_100.npy"
    ...
    for features_batch, labels_batch in train_dataset.take(1):
        outputs1 = alphago_sl_policy.predict(features_batch)
        print(outputs1)
        print(outputs1.shape)
        print("22342342")
        print(outputs1[1].shape)
        assert outputs1.shape[1:] == (361,), "Output shape does not match expected (None, 19, 19, 1)"
        sums = np.sum(outputs1, axis=-1)
        print('aaaa')
        print(sums)
...
def train():
    ...
    alphago_sl_policy.fit(train_dataset, 
                          ...
                          verbose=1,
                          callbacks=[ModelCheckpoint('../checkpoints/alphago{epoch}.keras')]
                          )    
