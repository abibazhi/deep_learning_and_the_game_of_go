...
def train_worker(learning_agent, output_file, experience_file,
    ...
    final_loss = learning_agent.train(exp_buffer, lr=lr, batch_size=batch_size)

    with h5py.File(output_file, 'w') as updated_agent_outf:
        print("这里呢？要保存下面内容，下面有没有呢？")
        print(output_file)
        learning_agent.serialize(updated_agent_outf)
    
    with open(f"chapter9/log.txt", 'a') as logf:
        logf.write(f"{output_file}的最后loss{final_loss}\n")
...
def main():
    ...
    parser.add_argument('--games-per-batch', '-g', type=int, default=960)
    ...
    while True:
        ...
        wins = evaluate(
            ...
            num_workers=args.num_workers,
            board_size=args.board_size)
        print('Won %d / 64 games (%.3f)' % (wins, float(wins) / 64.0))
        logf.write('Won %d / 64 games (%.3f)\n' % (wins, float(wins) / 64.0))
        # 这两个tmp_agent和working_agent都是文件路径。
        # 但是在拷贝的时候，是发生文件拷贝了；在赋值的时候，还是字符串变量。不是agent类型的变量。
