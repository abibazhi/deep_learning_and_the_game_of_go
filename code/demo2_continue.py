...
latest = sorted(glob.glob('./checkpoints/large_model_epoch_*.h5'), key=lambda x: int(x.split('_')[-1].split('.')[0]))
latest_model_path = latest[-1]  # 获取最后一个checkpoint文件路径

model = load_model(latest_model_path)  # 加载模型权重


batch_size = 128
# 继续训练
epochs = 50  # 假设原来总共打算训练50个epoch
